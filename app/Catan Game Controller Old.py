...
class Player(object):
  ...
  def __init__(self, id):
    p = models.Player.query.filter_by(id=id).first()
    self.id = id
    self.email = p.email
    self.name = p.name
    self.color = p.color
    self.resources = p.resources
    self.cities = p.cities
    self.settlements = p.settlements
    self.roads = set()
    self.devcards = {}
    self.cities_left = 4
  ...
  def save_all_to_db(self):
    if (self.id == None):
      game = Game(
          ...
          settup_round = self.settup_round,
...
class GameController(object):
  ...
  def add_player(self, email, name, color):
    if (len(players) < 4):
      p = Player(email, name, color)
      self.player_dictionary[email] = p
      self.players.append(p)

  def remove_player(self, email):
    if (name in self.player_dictionary):
      self.player_dictionary.delete(name)
    self.players = filter(lambda x: x.name != name, self.players)
